...
```{r, fig.align="center"}
knitr::include_graphics(normalizePath("~/Recon/PhD-Ideas/Swedish day wages/images/glass-production-1884.PNG"))
```

Because of the tabular form of the data and the high quality scan, we can use the [Layout parser tool](https://github.com/Layout-Parser/layout-parser) to grab the data and put it into a usable form in no time at all. Table \@ref(tab:glass-prod-tab) shows the parsed data, neat! We see that Kronoberg County within Småland has the greatest number of factories producing glass in 1884, at 11.
...
```{r, fig.align="center"}
knitr::include_graphics(normalizePath("~/Recon/PhD-Ideas/Swedish day wages/images/masters and journeymen 1831.PNG"))
```
...
If we plot a stacked area graph, we can see that exports and imports grow as a share of GDP after 1860, with the import band widening markedly prior to the First World War.
...
```{r}
knitr::include_graphics(normalizePath("~/Recon/PhD-Ideas/Swedish day wages/images/swedish-imports-1908-1928.PNG"))
```

Again due to the tabular nature of the data, it would be possible to use layout parser to extract this data into a machine readable format without much trouble.
